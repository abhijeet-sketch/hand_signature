...
class HandSignatureControl extends ChangeNotifier {
  ...
  String _exportPathSvg(
    ...
    data.forEach((line) {
      ...
      line.forEach(
          (path) => buffer.write(' C ${path.cpStart.dx} ${path.cpStart.dy}, ${path.cpEnd.dx} ${path.cpEnd.dy}, ${path.end.dx} ${path.end.dy}'));
      buffer.writeln('" />');
    });
  ...
  Picture toPicture({double scale = 1, Color? color, Color? background, double? size, double? maxSize, double? border}) {
    ...
    painter.paint(canvas, Size(width * scale, height * scale));
